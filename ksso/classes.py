...
class PortalController():
    ...
    class UserController():
        def __init__(self, user_data):
            ...
            # KAIST 단일서비스 인증정보가 저장되어 있는 경우 XML 정보를 바탕으로 database를 갱신합니다.
            try:
                self.portal_info = PortalInfo.objects.get(kaist_uid=self.kaist_uid)
                self.update_portal_info()

            # KAIST 단일서비스 인증정보가 저장되어 있지 않을 경우 PortalInfo instance를 생성합니다.
        ...
        def insert_portal_info(self):
            ...
            user = User.objects.create_user(username=self.kaist_uid, password=self.kaist_uid)
            user.save()
            self.portal_info = PortalInfo.create(user=user, kaist_uid=self.kaist_uid)
        ...
        class PortalParser():
            def __init__(self, user_data):
                ...
                self.data = ElementTree.fromstring(user_data.text).getchildren()[0].getchildren()[0].getchildren()[0]
    ...
    class AuthController():
        ...
        def build_request_string(self):
            request_string = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/"'
